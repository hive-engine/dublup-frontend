    ...
    <b-card title="Market question">
      <div class="market-question">
        ...
        <b-form-input
          v-model="person"
          class="inline-input"
          placeholder="Single Person or Single Group or Movie Title"
          :state="$v.person.$dirty ? !$v.person.$error : null"
        />
        win an award for <b-form-select
          v-model="award"
          class="inline-input"
          :options="computedAwards"
          :state="$v.award.$dirty ? !$v.award.$error : null"
...
import awardsMap from '@/assets/data/awards.json'
import timezones from '@/assets/data/timezones.json'

export default {
  name: 'EntertainmentAwardsBinaryTwo',
  ...
  computed: {
    ...
    calculatedEvents () {
      return [
        ...
        ...Object.keys(awardsMap)
      ]
    },

    computedAwards () {
      const awards = [{ vlaue: null, text: 'Award' }]

      if (this.event) {
        return [...awards, ...awardsMap[this.event]]
      }
  ...
  validations: {
    ...
    form: ['person', 'year', 'award', 'event', 'closeDate', 'closeTime', 'expiryDate', 'expiryTime']
  }
}
</script>
